...
import { Share, StyleSheet, Text, View, ScrollView, TextInput, SafeAreaView, Image, StatusBar, Pressable, TouchableOpacity, TouchableWithoutFeedback, Modal, KeyboardAvoidingView, Linking } from 'react-native'
...
const shareOnFacebook = () => {
    ...
    const fallbackURL = `https://www.facebook.com/sharer/sharer.php?${encodeURIComponent(appLink)}`;
...
const shareOnTwitter = () => {
    ...
    const fallbackURL = `https://twitter.com/intent/tweet?url=${encodeURIComponent(appLink)}`;

    Linking.canOpenURL(twitterAppURL).then(supported => {
        if (supported) {
...
const shareOnLinkedIn = () => {
    ...
    if (Platform.OS === 'ios') {
        linkedInAppURL = `linkedin://shareArticle?mini=true&url=${encodeURIComponent(appLink)}`;
    } else {
        linkedInAppURL = `linkedin://add?url=${encodeURIComponent(appLink)}`;
    }

    const fallbackURL = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(appLink)}`;

    Linking.canOpenURL(linkedInAppURL).then(supported => {
        if (supported) {
    ...
    return (
        <SafeAreaView style={styles.container}>
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <TouchableOpacity onPress={shareApp} style={{ marginTop: 50, width: '90%', alignSelf: 'center', height: 40, backgroundColor: '#0080FE', justifyContent: 'center', borderRadius: 10 }}>
                    <Text style={{ color: '#FFFFFF', fontWeight: '500', fontSize: 14, alignSelf: 'center' }}>Invite Friends</Text>
                </TouchableOpacity>


                <View style={{ paddingTop: '80%' }}>
                    <Text style={{ fontWeight: '500', fontSize: 14, alignSelf: 'center' }}>Share Via:</Text>
